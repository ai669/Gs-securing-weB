...
bool CWallet::UnlockStealthAddresses(const CKeyingMaterial& vMasterKeyIn)
{
    ...
    for (; mi != mapCryptedKeys.end(); ++mi)
    {
        ...
        CWayaWolfCoinAddress addr(ckid);
        ...
        if (fDebug)
        {
            ...
            CWayaWolfCoinAddress coinAddress(keyID);
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, int64_t nFees, CTransaction& txNew, CKey& key)
{
    ...
    // Set output amount
    // Standard stake
    if(txNew.vout.size() == 3){ // 2 stake outputs, stake was split
        txNew.vout[1].nValue = (blockValue / 2 / CENT) * CENT;
        txNew.vout[2].nValue = blockValue - txNew.vout[1].nValue;
    }
    else if(txNew.vout.size() == 2){ // only 1 stake output, was not split
...
bool CWallet::DelAddressBookName(const CTxDestination& address)
{
    ...
    if (!fFileBacked)
        return false;
    CWalletDB(strWalletFile).EraseName(CWayaWolfCoinAddress(address).ToString());
...
bool CWallet::ImportPrivateKey(CWayaWolfCoinSecret vchSecret, string strLabel, bool fRescan) {
    if (fWalletUnlockStakingOnly)
        return false;
