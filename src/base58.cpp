...
bool CWayaWolfCoinAddress::Set(const CKeyID &id) {
    SetData(Params().Base58Prefix(CChainParams::PUBKEY_ADDRESS), &id, 20);
    return true;
}

bool CWayaWolfCoinAddress::Set(const CScriptID &id) {
    SetData(Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS), &id, 20);
    return true;
}

bool CWayaWolfCoinAddress::Set(const CTxDestination &dest) {
    return boost::apply_visitor(CWayaWolfCoinAddressVisitor(this), dest);
}

bool CWayaWolfCoinAddress::IsValid() const {
...
CTxDestination CWayaWolfCoinAddress::Get() const {
    if (!IsValid())
...
bool CWayaWolfCoinAddress::GetKeyID(CKeyID &keyID) const {
...
bool CWayaWolfCoinAddress::IsScript() const {
    return IsValid() && vchVersion == Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS);
}

void CWayaWolfCoinSecret::SetKey(const CKey& vchSecret) {
...
CKey CWayaWolfCoinSecret::GetKey() {
    CKey ret;
    ret.Set(&vchData[0], &vchData[32], vchData.size() > 32 && vchData[32] == 1);
    return ret;
}

bool CWayaWolfCoinSecret::IsValid() const {
    bool fExpectedFormat = vchData.size() == 32 || (vchData.size() == 33 && vchData[32] == 1);
    bool fCorrectVersion = vchVersion == Params().Base58Prefix(CChainParams::SECRET_KEY);
    return fExpectedFormat && fCorrectVersion;
}

bool CWayaWolfCoinSecret::SetString(const char* pszSecret) {
    return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CWayaWolfCoinSecret::SetString(const std::string& strSecret) {
